...
def process_data(img_file, lbl_file, img_dest_folder, lbl_dest_folder, augment=False, sample_output_count=15):
    ...
    img_processor.grayscale().gaussian_blur(size=5, sigma=2.5).gradient_approximation().non_max_suppression().double_thresholding().hysteresis()

    # Get the final Canny edge-detected image
    img = img_processor._canny_img.astype(np.uint8)
